...
The audit's own words: *"the score barely moving while the code improved is
the audit getting sharper, not the work regressing."* An ever-sharpening bar
...
- [ ] **1.5 — Wire depthCostMultiplier into route selection** — the graded
  steer-away from unknown-depth water; the write exists
  (WeatherRoutingService:239), only the read is missing.
- [ ] **1.5 — Grounding-query edges** — fix the sub-231 m two-waypoint
  validation skip; stop guard-radius SOUNDG hits granting `insideCharted`
  (a lone sounding is not authoritative coverage — must not suppress GEBCO);
  surface the 5 m draft clamp to the user.
