/**
 * wavesGrid — fetches CMEMS ocean-waves hourly binaries produced
 * by scripts/cmems-waves-pipeline/pipeline.py.
 *
 ...
 *   /api/waves/manifest.json  →  Vercel edge fn → GitHub Release asset
 *   /api/waves/h00.bin        →  Vercel edge fn → GitHub Release asset
 * The edge function lives at `api/waves/[file].ts`; dev uses the
 * matching `/api/waves` proxy in vite.config.ts.
 *
 * Why `/api/waves` and not `/waves`: Vercel's Attack Challenge Mode
 ...
 *
 * Binary layout (see scripts/cmems-waves-pipeline/pipeline.py):
...
export async function fetchWavesManifest(): Promise<WavesManifest | null> {
    try {
        ...
        return (await res.json()) as WavesManifest;
    } catch (err) {
 ...
 */
export async function fetchWavesGrid(): Promise<WindGrid | null> {
    if (cachedGrid) return cachedGrid;
    if (inflight) return inflight;
    inflight = doFetchWavesGrid().finally(() => {
        inflight = null;
    });
...
async function doFetchWavesGrid(): Promise<WindGrid | null> {
    ...
    log.info(
        `Loaded waves: ${hourCount}h × ${width}×${height}, bounds n=${north} s=${south} w=${west} e=${east}` +
