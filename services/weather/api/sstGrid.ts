/**
 * sstGrid — fetches CMEMS ocean-sst hourly binaries produced
 * by scripts/cmems-sst-pipeline/pipeline.py.
 *
 ...
 *   /api/sst/manifest.json  →  Vercel edge fn → GitHub Release asset
 *   /api/sst/h00.bin        →  Vercel edge fn → GitHub Release asset
 * The edge function lives at `api/sst/[file].ts`; dev uses the
 * matching `/api/sst` proxy in vite.config.ts.
 *
 * Why `/api/sst` and not `/sst`: Vercel's Attack Challenge Mode
 ...
 *
 * Binary layout (see scripts/cmems-sst-pipeline/pipeline.py):
...
export async function fetchSstManifest(): Promise<SstManifest | null> {
    try {
        ...
        return (await res.json()) as SstManifest;
    } catch (err) {
 ...
 */
export async function fetchSstGrid(): Promise<WindGrid | null> {
    if (cachedGrid) return cachedGrid;
    if (inflight) return inflight;
    inflight = doFetchSstGrid().finally(() => {
        inflight = null;
    });
...
async function doFetchSstGrid(): Promise<WindGrid | null> {
    ...
    log.info(
        `Loaded sst: ${hourCount}h × ${width}×${height}, bounds n=${north} s=${south} w=${west} e=${east}` +
    ...
    return {
        ...
        hourOffsets,
        // Pipeline run time — day 0 of the SST forecast. Used by
