...
/**
 * Generate a yacht-skipper-grade country brief: visa, biosecurity,
 * ports of entry, and one yacht-specific note (advance form, VHF
 * channel, agent requirement).
 *
 * Used by CountrySnippetService as the fallback for countries not in
 * the hand-curated list. The result MUST be marked AI-generated by
 * the caller — these are safety/legal facts and the model can be
 * outdated. The prompt biases toward "if unsure, say verify with
 * consulate" to keep wrong-confidence answers rare.
 *
 * Returns null on any failure (network, parse, model unavailable);
 * caller should fall back to a generic stub.
 */
export const fetchCountryYachtBrief = async (
    countryName: string,
): Promise<{
    visa: string;
    biosecurity: string;
    ...
    if (!isGeminiConfigured()) return null;
    if (!countryName || countryName.length < 2) return null;
    try {
        const prompt = `You are a maritime compliance assistant for a yacht skipper arriving by sea at: "${countryName}".

Provide CONCISE, practical facts. Return STRICT JSON with exactly these four string fields:

{
  "visa": "1–2 sentences. Visa rules for AU/NZ citizens arriving by yacht. Include default duration. If a pre-arrival eVisa or ETA is required, say so. If unsure of current rules, say 'Confirm current rules with the consulate before departure.'",
  "biosecurity": "1–2 sentences. What's restricted on arrival (fresh produce, meat, dairy, seeds, pets). Include strictness level: Strict / Moderate / Light. If unsure say 'Declare all produce; confirm specifics with customs.'",
  "portsOfEntry": "Comma-separated list of the 1–4 most-used yacht ports of entry. Use the canonical English name.",
  "notes": "1 sentence. The single most important yacht-specific tip — advance notice form, VHF clearance channel, agent requirement, mooring vs anchoring rules, etc. If nothing notable, say 'Standard yacht clearance procedures apply.'"
}

Stick to widely-known facts. Do not speculate about politics, embargoes, or sanctions. Output JSON only — no prose.`;

        const text = await callGeminiProxy({
            prompt,
            responseMimeType: 'application/json',
            // Lower temperature — facts, not creative writing.
            temperature: 0.2,
            // Generous cap so the JSON never gets truncated mid-string.
            maxTokens: 800,
        });

        const res = cleanAndParseJson<{
            visa?: unknown;
            biosecurity?: unknown;
            portsOfEntry?: unknown;
            notes?: unknown;
        }>(text || '{}');
        if (!res) return null;

        // Defensive — every field must be a non-empty string. Reject
        // partial responses so the caller falls through to the stub.
        const need = (v: unknown): v is string => typeof v === 'string' && v.trim().length > 0;
        if (!need(res.visa) || !need(res.biosecurity) || !need(res.portsOfEntry) || !need(res.notes)) {
            return null;
        }
        return {
            visa: res.visa.trim(),
            biosecurity: res.biosecurity.trim(),
            portsOfEntry: res.portsOfEntry.trim(),
