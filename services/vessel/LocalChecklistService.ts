 ...
 */
import { getAll, insertLocal, updateLocal, deleteLocal, generateUUID } from './LocalDatabase';
...
export class LocalChecklistService {
    ...
    static getDetailsByHeading(headingId: string): ChecklistEntry[] {
        return LocalChecklistService.getAll().filter((e) => e.type === 'detail' && e.heading_id === headingId);
    }
    ...
    static getGrouped(): { heading: ChecklistEntry; items: ChecklistEntry[] }[] {
        ...
        return headings.map((h) => ({
            heading: h,
            items: all.filter((e) => e.type === 'detail' && e.heading_id === h.id).sort((a, b) => a.order - b.order),
        }));
    }
    ...
    static async create(entry: Pick<ChecklistEntry, 'type' | 'text' | 'heading_id'>): Promise<ChecklistEntry> {
        const all = LocalChecklistService.getAll();
    ...
    static async update(id: string, updates: Partial<ChecklistEntry>): Promise<ChecklistEntry | null> {
    ...
    static async delete(id: string): Promise<void> {
        ...
        if (entry?.type === 'heading') {
            const children = getAll<ChecklistEntry>(TABLE).filter((e) => e.type === 'detail' && e.heading_id === id);
