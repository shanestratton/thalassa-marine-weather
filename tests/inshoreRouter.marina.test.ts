...
describe('Fairlead — end-to-end through routeInshore', () => {
    ...
    it('splices the lateral-mark centreline when the route transits a marked channel', () => {
        // Navigable band (deep DEPARE) at region ~156. A buoyed channel runs
        // along it: green/starboard marks on the south edge, red/port on the
        // north edge, sequence-numbered. A route straight along the band
        // transits the channel → Fairlead should fire.
        const deep = fc(rect(156.04, -27.206, 156.16, -27.194, { DRVAL1: 12.0 }));
        // Marks ~1 km apart (within groupChannels' 1.5 km clustering range —
        // real channel marks are ~1.2 km apart; an earlier 2.5 km spacing
        // failed to cluster, which is correct behaviour).
        ...
        expect(isResult(r)).toBe(true);
        if (!isResult(r)) return;
        expect(r.debug?.fairlead).toBe('BC'); // the buoyed channel was followed
    });

    it('no lateral marks → route unchanged (Fairlead no-ops)', () => {
        const deep = fc(rect(156.34, -27.206, 156.46, -27.194, { DRVAL1: 12.0 }));
        const r = routeInshore(
            { DEPARE: deep },
        ...
        expect(isResult(r)).toBe(true);
        if (!isResult(r)) return;
        expect(r.debug?.fairlead).toBeUndefined();
    });
});
