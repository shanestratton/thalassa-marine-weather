...
describe('soundingDensity — the numbers ladder', () => {
    it('the shallowest sounding claims the coarse zooms; a deep neighbour waits', async () => {
        ...
        await assignSoundingDensityMinZoom([deep, shallow]);
    ...
    it('far-apart soundings both appear at the coarsest zoom', async () => {
        const a = pt(153.0, -27.0, 12);
        const b = pt(155.0, -25.0, 15); // degrees apart — separate z4 cells
        await assignSoundingDensityMinZoom([a, b]);
        expect(mz(a)).toBe(4);
        expect(mz(b)).toBe(4);
    });

    it('density fills in progressively with zoom, not all-at-once', async () => {
        ...
        await assignSoundingDensityMinZoom(cluster);
    ...
    it('grid is screen-square at high latitude — no column-y Mercator bias', async () => {
        ...
        await assignSoundingDensityMinZoom([a, vert]);
        expect(mz(a)).toBe(4);
        expect(mz(vert)).toBe(4); // its own screen cell — shows immediately
        const a2 = pt(150.0, -60.0, 5);
        await assignSoundingDensityMinZoom([a2, horiz]);
        expect(mz(a2)).toBe(4);
        expect(mz(horiz)).toBeGreaterThan(4); // same screen cell — waits
    });

    it('CAP: past the 80k rung-assign cap the deepest tail is past-the-ladder; shallow keeps its rung', async () => {
        ...
        await assignSoundingDensityMinZoom(heap);
    ...
    it('REPLACES the SCAMIN pre-bake — wide-zoom numbers actually appear', async () => {
        ...
        await assignSoundingDensityMinZoom([gated]);
