...
export function downloadRouteGPX(voyagePlan: VoyagePlan): void {
    ...
    const origin =
        typeof voyagePlan.origin === 'string' ? voyagePlan.origin.split(',')[0].replace(/\s+/g, '_') : 'Origin';
    const dest =
        typeof voyagePlan.destination === 'string'
            ? voyagePlan.destination.split(',')[0].replace(/\s+/g, '_')
            : 'Destination';
