...
async function generateInventoryPDF({ items, categories, vesselName }: InventoryPdfOptions): Promise<JsPDFType> {
    ...
    const cats = categories && categories.size > 0
    ...
    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'normal');
    const catLabel = categories && categories.size > 0
    ...
    pdf.text(`${filteredItems.length} Items · ${totalQty} Units${lowStock > 0 ? ` · ${lowStock} Low Stock` : ''}`, W / 2, 38, { align: 'center' });
