-- Allow authenticated sailors to test private messages with themselves.
-- Self blocks affect only that private conversation; peer consent, payload,
-- moderation, visibility and bilateral block enforcement stay unchanged.
BEGIN;
...
BEGIN
    IF auth.role() IS DISTINCT FROM 'authenticated' OR owner_id IS NULL THEN
        RAISE EXCEPTION 'Authentication required' USING ERRCODE = '42501';
    END IF;
    IF p_other_user_id IS NULL OR p_blocked IS NULL THEN
        RAISE EXCEPTION 'Invalid block target' USING ERRCODE = '22023';
    END IF;
    ...
    IF p_blocked THEN
        ...
        -- A self-test block must not hide the skipper's own public posts or
        -- create a platform/moderation block. Peers still mirror both stores.
        IF p_other_user_id <> owner_id THEN
...
-- A separate, exact-self permission leaves the existing peer-consent policy
-- byte-for-byte intact. The restrictive bilateral-block guard still ANDs this
-- permission, so a self block is enforced on the server, not just by the UI.
CREATE POLICY chat_direct_messages_self_test
ON public.chat_direct_messages FOR INSERT TO authenticated
WITH CHECK (
    ...
    AND NOT EXISTS (
        SELECT 1 FROM public.chat_roles chat_role
        WHERE chat_role.user_id = (SELECT auth.uid())
          AND (COALESCE(chat_role.is_blocked, false) OR chat_role.muted_until > now())
    )
);

-- No data cleanup, peer-policy replacement, new SELECT grant or safety bypass.
COMMIT;
