...
export const VoyageCard: React.FC<{
    ...
    /**
     * Show the one-line "swipe left to archive or delete" hint under this
     * card. LogPage passes it for the first card only; it disappears for
     * good after the first swipe on any card.
     */
    ...
    ({
        ...
        // Memoised, and skipped while collapsed: the value is only read in the
        // expanded branch, so N collapsed cards must not each pay
        // O(|filteredEntries|) on every LogPage render (returning `entries`
        // there is safe — nothing reads it).
        const voyageFilteredEntries = React.useMemo(() => {
            if (!isExpanded || entries.length === 0) return entries;
            const filteredIds = new Set(filteredEntries.map((f) => f.id));
            return entries.filter((e) => filteredIds.has(e.id));
        }, [isExpanded, entries, filteredEntries]);
        // Each of these is an O(n) pass over a voyage's points (thousands at
        // the 2 Hz capture cadence) — once per data change, not per render.
        const propulsionSplit = React.useMemo(
            () => (isExpanded && !isPlannedRoute ? computePropulsionSplit(voyageFilteredEntries) : null),
            [isExpanded, isPlannedRoute, voyageFilteredEntries],
        );
        const noonRuns = React.useMemo(
            () => (isExpanded && !isPlannedRoute ? groupEntriesByNoonWindow(voyageFilteredEntries) : []),
            [isExpanded, isPlannedRoute, voyageFilteredEntries],
        );
        const groupedEntries = React.useMemo(
            () => (isExpanded ? groupEntriesByDate(voyageFilteredEntries) : []),
            [isExpanded, voyageFilteredEntries],
        );
        ...
        return (
            <>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="flex items-start justify-between mb-2">
                                {/* Expand chevron — marks the tap zone that toggles the
                                timeline (the left third) and shows its state. */}
                                <svg
                                    className={`shrink-0 w-3.5 h-3.5 mt-1 mr-1.5 text-slate-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    stroke="currentColor"
                                    strokeWidth={2.5}
                            ...
                            {isPlannedRoute && (
                                <div className="text-[10px] text-purple-400/80 mt-1.5">
                ...
                {showSwipeHint && !swipeHintSeen && (
                    <p className="-mt-2 mb-3 px-3.5 text-[11px] text-slate-500">
                        Swipe a voyage left to archive or delete
                    </p>
                )}
            </>
        );
