...
import { resolveBoundingBoxes } from './boundingBox.js';
...
// code-side change can sit inert for a week without anyone noticing.
console.log(`[AIS] Bounding box in force: ${JSON.stringify(BOUNDING_BOXES)} (${BOUNDING_BOXES_SOURCE})`);
// Say which box is actually in force, and whether it came from the dashboard
// or from this file. Without this line the only way to know an override is
// winning is to read the Railway variables tab and trust it.
...
/**
 * AISSTREAM IS OPTIONAL, and refusing to boot without it was a real outage.
 *
 * This used to be `process.exit(1)`. aisstream.io has been dead service-wide
 * since 2026-08-05 (their issue #269 and a dozen like it, no maintainer
 * replies), so its key is a credential for a service that no longer works —
 * and the moment it was tidied out of the Railway variables the worker began
 * exiting on boot. Nothing then served /health, Railway's healthcheck failed,
 * and the deploy was marked dead (Shane, 2026-08-24).
 *
 * Everything else in this worker is independent of aisstream: the punter
 * crowd-feed, the AISHub aggregate poller, the DB flush loop and the health
 * endpoint. Taking all of them down over one absent key for one dead upstream
 * is the wrong trade. Absent the key we simply do not open the socket.
 */
const AISSTREAM_ENABLED = Boolean(API_KEY);
if (!AISSTREAM_ENABLED) {
    console.log('[AIS] AISSTREAM_KEY not set — aisstream feed disabled. Every other subsystem runs as normal.');
}
...
const healthServer = http.createServer((req, res) => {
    ...
    if (req.url === '/health' || req.url === '/') {
        ...
        const body = JSON.stringify({
            ...
            // any success).
            // With aisstream disabled, lastMessageAt never advances, so the
...
// The dead-man switch only means something when there is a socket to revive.
if (AISSTREAM_ENABLED) setInterval(checkStaleConnection, STALE_CHECK_INTERVAL_MS);
