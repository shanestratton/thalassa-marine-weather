...
export const VoyageHeader: React.FC<VoyageHeaderProps> = React.memo(
    ({ voyageId, entries, isActive, isSelected, isExpanded, onToggle, onSelect, onDelete }) => {
        ...
        useEffect(() => {
            const fetchLocationNames = async () => {
                ...
                if (
                    firstEntry &&
                    isSentinelName(firstEntry.waypointName) &&
                    (firstEntry.latitude !== 0 || firstEntry.longitude !== 0)
                ) {
                    try {
                ...
                if (
                    lastEntry &&
                    isSentinelName(lastEntry.waypointName) &&
                    lastEntry.id !== firstEntry?.id &&
                    (lastEntry.latitude !== 0 || lastEntry.longitude !== 0)
                ) {
                    try {
                        ...
                        <div className="px-2.5 py-2">
                            ...
                            {entries.some((e) => e.source && e.source !== 'device') && (
                                <div className="flex items-center gap-1 mt-0.5">
                                    ...
                                    <p className="text-sm text-amber-500/70 whitespace-nowrap">
                                        Not for navigation — unverified data. Verify independently.
                                    </p>
                                </div>
                            )}
