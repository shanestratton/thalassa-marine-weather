    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-4 mt-2 mb-4 animate-in fade-in slide-in-from-top-2 duration-500">
            <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-2 mask-linear">
                <div className="flex items-center justify-center w-8 h-8 rounded-full bg-white/5 shrink-0 border border-white/5">
                ...
                {favorites.map((loc, idx) => (
                    <div 
                        ...
                        className="group flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 hover:bg-sky-500/20 border border-white/10 hover:border-sky-500/30 transition-all cursor-pointer whitespace-nowrap active:scale-95 select-none"
                    >
                        ...
                        <span className="text-xs font-medium text-gray-300 group-hover:text-white transition-colors">{loc}</span>
                        <button 
                            onClick={(e) => {
                                e.stopPropagation();
                                onRemove(loc);
