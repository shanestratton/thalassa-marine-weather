/**
 * WaveParticleLayer — Mapbox custom WebGL layer for ocean-waves particle
 * animation. Forked from CurrentParticleLayer. Same rendering engine,
 * wave-tuned constants: the vector field is VHM0×direction (pipeline
 * packs it as u/v so we can reuse the whole heatmap + LINES pipeline),
 ...
 *     particles needed there.
 *   – SPEED_SLACK/STRONG retuned for wave-height ramp: 0.5m (gentle
 *     swell) → 4m (rough sea), heatmap deep-coral above.
 *
 * Everything else (speed-weighted spawn, land-mask requirement, heatmap
 * underlay, inverse-Mercator UV, antimeridian wrap reset, GPU LINES
 * streaks, always-draw / advect-throttled render loop) is identical.
 * We'll likely refactor the two layers onto a shared VectorFieldParticle
 * Layer once SST + chlorophyll + sea-ice + MLD are in and the right
...
export class WaveParticleLayer implements mapboxgl.CustomLayerInterface {
    readonly id: string;
     ...
     *  respawn weighted by the new height field. */
    setWaves(
        u: Float32Array,
        v: Float32Array,
        width: number,
    ...
    private weightedSpawn(): [number, number] {
        ...
        // Diagnostic: window.__wavesUniformSpawn = true → skip the
        ...
        if (dbg.__wavesUniformSpawn === true && mask && w > 0 && h > 0) {
    ...
    render(gl: WebGLRenderingContext, matrixOrOptions: unknown): void {
        ...
        //   window.__wavesShowHeatmap = false  // hide speed-magnitude raster
        //   window.__wavesShowLines   = false  // hide particle streaks
        // Default: both true. Requires a repaint after setting (zoom 1 px
        // or toggle the layer off+on).
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const dbg = globalThis as any;
        const showHeatmap = dbg.__wavesShowHeatmap !== false;
        const showLines = dbg.__wavesShowLines !== false;
