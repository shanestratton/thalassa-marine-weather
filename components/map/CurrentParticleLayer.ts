 ...
 *     0.02 kt kill threshold, up to 80k device-tiered particles, 20-frame trails.
...
export class CurrentParticleLayer implements mapboxgl.CustomLayerInterface {
    ...
    onAdd(map: mapboxgl.Map, gl: WebGLRenderingContext): void {
        ...
        try {
            ...
            beginWebGlOperation(gl, 'CurrentParticleLayer', 'initialisation');
            this.program = createWebGlProgram(gl, 'CurrentParticleLayer', PARTICLE_VERT, PARTICLE_FRAG, 'particle');
            ...
            this.particleBuffer = requireWebGlResource(
                gl.createBuffer(),
                'CurrentParticleLayer',
            ...
            proveWebGlOperation(gl, 'CurrentParticleLayer', 'particle trail buffer upload');
            ...
            if (gl2.createVertexArray) {
                ...
                proveWebGlOperation(gl, 'CurrentParticleLayer', 'particle vertex array setup');
            }
            ...
            if (uint32Ext) {
                ...
                proveWebGlOperation(gl, 'CurrentParticleLayer', 'particle line index upload');
            ...
            // ── Heatmap underlay ─────────────────────────────────────────
            this.heatmapProgram = createWebGlProgram(gl, 'CurrentParticleLayer', HEATMAP_VERT, HEATMAP_FRAG, 'heatmap');
            ...
            proveWebGlOperation(gl, 'CurrentParticleLayer', 'heatmap vertex upload');
            ...
            proveWebGlOperation(gl, 'CurrentParticleLayer', 'heatmap index upload');
            this.heatmapIndexCount = indexCount;

            this.speedTexture = requireWebGlResource(gl.createTexture(), 'CurrentParticleLayer', 'speed texture');
            proveWebGlOperation(gl, 'CurrentParticleLayer', 'speed texture allocation');
            ...
            try {
                ...
                // Preserve the allocation failure; onRemove is idempotent and
                // Mapbox may invoke it again when the partial ID is removed.
            }
            throw error;
        }
    }
    ...
    setCurrents(
        ...
        if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width <= 0 || height <= 0) {
            throw new Error('[CurrentParticleLayer] grid dimensions must be positive safe integers');
        }
        if (u.length !== width * height || v.length !== width * height) {
            throw new Error(
                `[CurrentParticleLayer] size mismatch: u=${u.length} v=${v.length} expected=${width * height}`,
            );
        }
        if (landMask.length !== width * height) {
            throw new Error(
                `[CurrentParticleLayer] land mask size mismatch: ${landMask.length} expected=${width * height}`,
            );
        }
        if (
            !this.map ||
            ...
            throw new Error('[CurrentParticleLayer] renderer is not fully initialised');
        }
        this.clearFrameData();
        try {
            this.ensureCpuParticleState();
    ...
    private uploadSpeedTexture(): void {
        ...
        if (!gl || !tex || !speed || !mask) {
            throw new Error('[CurrentParticleLayer] speed texture upload has incomplete renderer data');
        }
        ...
        beginWebGlOperation(gl, 'CurrentParticleLayer', 'speed texture upload');
        ...
        try {
            ...
            proveWebGlOperation(gl, 'CurrentParticleLayer', 'speed texture upload');
        } finally {
