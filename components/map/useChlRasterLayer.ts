...
import { fetchChlGrid, releaseChlGrid } from '../../services/weather/api/chlGrid';
...
const FEATURE_ENABLED = isCmemsFeatureEnabled('chl');
    ...
    useEffect(() => {
        ...
        if (!visible || !grid) {
            ...
            if (deactivation === 'absent') {
                ...
                return monitorCmemsLayerDeactivation(map, LAYER_ID, 'Chlorophyll', publishSafeDeactivation);
            } else {
                setRenderOutcome({
                    phase: 'stuck-visible',
                    attempt: refresh.attempt,
        ...
        if (!grid.landMask) {
            log.warn('Verified CHL grid has no land mask — skipping draw');
        ...
        if (grid.sourceStep !== wantsStep || !grid.u[wantsStep]) {
            log.warn('Verified CHL frame does not match the requested scrubber step');
            return deactivateFailedCmemsRenderer({
                map,
                layerId: LAYER_ID,
                label: 'Chlorophyll',
        ...
        if (!layerRef.current) {
            ...
            try {
                ...
                if (!addCmemsLayerAndProveOwnership(map, LAYER_ID, layer)) {
                    throw new Error('Mapbox did not register the CHL candidate');
                }
                currentStepRef.current = -1;
