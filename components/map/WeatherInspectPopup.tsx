        ...
        <div style={{ minWidth: 240, maxWidth: 280 }}>
            ...
            >
                ...
                <button
                    ...
                    aria-label="Close"
                >
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-white/50">
                ...
                <div className="flex items-center gap-1.5 mb-2 pr-6">
                    <span className="text-[10px] text-sky-400/70 font-mono font-bold">
                        {data ? `${Math.abs(data.lat).toFixed(2)}°${data.lat >= 0 ? 'N' : 'S'} ${Math.abs(data.lon).toFixed(2)}°${data.lon >= 0 ? 'E' : 'W'}` : '…'}
                    </span>
                </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 gap-x-3">
                            <Metric
                                icon="🌊"
                                label="Waves"
                                value={data && data.waveHeightM != null ? `${data.waveHeightM.toFixed(1)}m / ${metresToFeet(data.waveHeightM)}ft` : ''}
                                loading={loading}
                            />
                            <Metric
                                icon="⏱"
                            ...
                            {data && data.swellHeightM != null && data.swellHeightM > 0 && (
                                <>
                                    ...
                                    <Metric
                                        ...
                                        sub={data.swellDirectionDeg != null ? degToCardinal(data.swellDirectionDeg) : ''}
                                        loading={loading}
                                    />
                                </>
                            )}
