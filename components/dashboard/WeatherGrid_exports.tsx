...
export const MetricsWidget = ({
    ...
    displayValues: CardDisplayValues;
}) => {
    ...
    return (
        <div className="grid grid-cols-3 gap-4">
            <Card
                className={`col-span-1 border transition-colors ${isSensorLocked ? 'border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.1)]' : 'border-white/10'}`}
            >
                <div className="absolute top-2 right-2 flex gap-1">
                    {isSensorLocked && (
                        <span className="text-[10px] font-bold bg-emerald-500 text-white px-1 py-0.5 rounded-sm leading-none uppercase tracking-tighter">
                            Verified
                        </span>
                    )}
                </div>
                <Metric
                    ...
                    label={`Wind (${units.speed})`}
                    value={`${displayValues.windSpeed}`}
                    subValue={<span className="text-amber-300 font-medium">Gusting: {displayValues.gusts}</span>}
            ...
            <Card
                className={`col-span-1 border transition-colors ${isSensorLocked ? 'border-emerald-500/30' : 'border-white/10'}`}
            >
                <Metric
