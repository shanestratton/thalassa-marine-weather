        ...
        <Card className="bg-slate-900/60 border border-white/10">
            <div className="space-y-3">
                ...
                <div
                    ...
                    className="w-full overflow-x-auto snap-x snap-mandatory custom-scrollbar pb-6 flex gap-4 touch-pan-x"
                >
                    {hourly &&
                        hourly.map((item: HourlyForecast, idx: number) => {
                            ...
                            if (isLandlocked) {
                                ...
                                waveCell = (
                                    <div className="bg-white/5 rounded-2xl p-3 border border-white/5 flex flex-col justify-between">
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="text-[11px] uppercase text-gray-400 font-bold">Wave</span>
                                        </div>
                                        <div className="flex items-baseline gap-1">
                                ...
                                if (tideStatus === 'rising') {
                                    ...
                                    tideBg =
                                        'bg-purple-500/20 border-purple-500/30 shadow-[0_0_10px_rgba(192,132,252,0.1)]';
                                }

                                const tideVal =
                                    item.tideHeight !== undefined
                                        ? convertLength(item.tideHeight, units.tideHeight || 'm')
                                        : null;
                                    ...
                                    <div className={`rounded-2xl p-3 border flex flex-col justify-between ${tideBg}`}>
                                        ...
                                        <div className="h-1 w-full bg-black/20 rounded-full mt-2 overflow-hidden">
                                            <div
                                                ...
                                                style={{
                                                    width: `${Math.min(((item.tideHeight || 0) / 3) * 100, 100)}%`,
                                                }}
                                            ></div>
                                        </div>
                                    </div>
                            ...
                            return (
                                <div
                                    key={idx}
                                    className="flex-none w-full sm:w-[calc(50%-0.5rem)] lg:w-[calc(33.333%-0.67rem)] snap-start bg-slate-900/80 rounded-2xl p-5 border border-white/10 hover:bg-slate-900 transition-colors shadow-lg"
                                >
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="relative">
                                                ...
                                                {cloudCover !== undefined && cloudCover !== null && (
                                                    <div className="absolute -top-2 -right-2 text-[11px] text-white font-bold bg-white/20 px-1.5 py-0.5 rounded-full">
                                                        {cloudCover}%
                                                    </div>
                                                )}
                                            </div>
                                            <div className="text-right">
                                                <p
                                                    className={`text-3xl font-light ${item.isEstimated ? 'text-yellow-400' : 'text-white'}`}
                                                >
                                                    {tempDisplay}°
                                                </p>
                                                {chillDisplay && (
                                                    <p className="text-[11px] text-gray-400">Feels {chillDisplay}°</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
            ...
            <div className="space-y-4 relative">
                {forecast.map((day: ForecastDay, i: number) => {
                    ...
                    return (
                        <div
                            key={i}
                            className={`flex flex-col bg-slate-900/40 hover:bg-slate-900/60 border border-white/5 rounded-2xl p-5 gap-4 transition-all shadow-md ${isLocked ? 'blur-md opacity-30 pointer-events-none select-none grayscale' : ''}`}
                        >
                            <div className="flex justify-between items-center border-b border-white/5 pb-3">
                                <div className="flex items-center gap-4">
                                    <WeatherIcon
                                        ...
                                        className="w-12 h-12 text-sky-300 drop-shadow-md"
                                    />
                                    <div>
                                ...
                                <div
                                    className={`px-4 py-1.5 rounded-xl border flex flex-col items-center justify-center min-w-[90px] h-full ${scoreClass}`}
                                >
                                    <span className="text-2xl font-mono font-bold leading-none">{score}</span>
                                    <span className="text-[11px] font-bold uppercase tracking-wide leading-none mt-1 text-center">
                                        {scoreLabel}
                                    </span>
                                    <span className="text-[11px] uppercase tracking-wider opacity-80 mt-0.5 font-bold border-t border-current/20 pt-0.5 w-full text-center">
                                        {condText}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                <div className="bg-white/5 rounded-xl p-3 border border-white/5 relative overflow-hidden group flex flex-col justify-between">
                                    ...
                                    <div className="flex justify-between items-center mb-1 relative z-10">
                                        <span className="text-[11px] text-gray-400 uppercase font-bold">Wind</span>
                                    </div>
                                ...
                                <div className="bg-white/5 rounded-xl p-3 border border-white/5 relative overflow-hidden group flex flex-col justify-between">
                                    ...
                                    <div className="pt-2 border-t border-white/10 flex justify-between items-center">
                                        ...
                                        <span className="text-xs text-sky-300 font-mono font-bold">
                                            {day.swellPeriod ? `${day.swellPeriod}s` : '--'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {hasTideData ? (
                                    <div className="bg-white/5 rounded-xl p-3 border border-white/5 flex flex-col justify-between">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[11px] text-gray-400 uppercase font-bold">Tide</span>
                                            <TideCurveIcon className="w-4 h-4 text-purple-400" />
                                        </div>
                                        <div className="text-sm font-medium text-white truncate my-1">
                                            {day.tideSummary}
                                        </div>
                                        {day.sunrise && (
                                            <div className="flex justify-between items-center mt-1 pt-1 border-t border-white/5 text-[11px] text-gray-400">
                                                <span className="flex items-center gap-1">
                                                    <SunIcon className="w-3 h-3 text-amber-400" /> {day.sunrise}
                                                </span>
                                    ...
                                    <div className="bg-white/5 rounded-xl p-3 border border-white/5 flex flex-col justify-between">
                                        ...
                                        <div className="mt-2 pt-1 border-t border-white/5 text-[11px] text-gray-400 text-center uppercase tracking-wide">
                                            Daylight Hours
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="bg-white/5 rounded-xl p-3 border border-white/5 flex flex-col justify-between">
                                    ...
                                    <div className="flex justify-between items-center pt-1 border-t border-white/5">
                                        <span className="text-[11px] text-gray-400 uppercase font-bold">UV Index</span>
                                        <span
    ...
    <Card className="p-0 overflow-hidden h-60 cursor-pointer group hover:border-sky-500/50 transition-colors relative">
        <div onClick={onOpenMap} role="button" tabIndex={0} className="w-full h-full relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10">
                ...
                <h3 className="text-2xl font-bold text-white mb-1 tracking-tight group-hover:text-sky-200 transition-colors">
                    Interactive Weather Map
                </h3>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-widest group-hover:text-gray-300">
                    Live GRIB Visualization & Observations
                </p>
            </div>
