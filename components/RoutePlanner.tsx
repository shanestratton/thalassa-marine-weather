        ...
        <div className="relative flex-1 bg-slate-950 overflow-hidden flex flex-col">
                    ...
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-xs font-bold text-amber-400 uppercase tracking-wider">
                                Route Calculation Temporarily Disabled
                            </p>
                            <p className="text-[11px] text-amber-300/60 mt-0.5">
                                Awaiting Navionics / i-Boating chart SDK approval. OpenSeaMap overlay is active on the
                                4D map.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="fixed inset-0 z-[2000] bg-slate-900 flex flex-col">
                        <div className="relative flex-1">
                            <MapHub
                                ...
                                pickerLabel={
                                    mapSelectionTarget
                                        ? `Tap to select ${mapSelectionTarget === 'origin' ? 'Origin' : mapSelectionTarget === 'destination' ? 'Destination' : 'Via Waypoint'}`
                                        : undefined
                                }
                                initialZoom={8}
                                center={
            ...
            {voyagePlan ? (
                                ...
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-sky-500/10 rounded-lg border border-sky-500/20">
                                            {vessel?.type === 'power' ? (
                                                <PowerBoatIcon className="w-5 h-5 text-sky-400" />
                                            ) : (
                                                <SailBoatIcon className="w-5 h-5 text-sky-400" />
                                            )}
                                        </div>
                                        <div>
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setTimeout(() => {
                                                        window.dispatchEvent(
                                                            new CustomEvent('thalassa:passage-mode', { detail }),
                                                        );
                                                    }, 200);
                                                }}
                        ...
                        {error && (
                            <div className="max-w-4xl mx-auto mb-8 w-full animate-in fade-in slide-in-from-top-4">
                                <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-xl flex items-center gap-4 text-red-200">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-sm uppercase">Calculation Error</h4>
                ...
                >
                    ...
                    <form
                        ref={formRef}
                        onSubmit={ROUTING_DISABLED ? (e) => e.preventDefault() : handleCalculate}
                        className="shrink-0 px-4 pt-4 overflow-y-auto flex-1 min-h-0"
                    >
                        <div
                            className={`max-w-xl mx-auto w-full space-y-3 ${ROUTING_DISABLED ? 'opacity-40 pointer-events-none' : ''}`}
                        >
                            {/* Origin */}
                            ...
                            <div className="relative group">
                                ...
                                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                    <button
                                        ...
                                        className="p-2 text-gray-400 hover:text-sky-400 transition-colors hover:bg-white/10 rounded-lg"
                                        title="Select on Map"
                                        aria-label="Select via point on map"
                                    >
                                        <MapIcon className="w-4 h-4" />
                                    </button>
                                    {via && (
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        if (result.bestDepartureWindow) {
                                            ...
                                            if (!bdw.dateTimeISO) {
                                                const timeMatch =
                                                    bdw.timeRange?.match(/(\d{1,2}):?(\d{2})?\s*(am|pm)?/i);
                                                if (timeMatch) {
                                            ...
                                            const dateStr = bdw.dateTimeISO
                                                ? new Date(bdw.dateTimeISO).toLocaleDateString('en-AU', {
                                                      weekday: 'short',
                                                      day: 'numeric',
                                                      month: 'short',
                                                      hour: '2-digit',
                            ...
                            {suggestedReasoning && (
                                <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl px-3 py-2 animate-in fade-in slide-in-from-top-2">
                                    <p className="text-[11px] text-emerald-300/80 leading-relaxed whitespace-pre-line">
                                        <span className="font-bold text-emerald-300">⚡ AI: </span>
                                        {suggestedReasoning}
                                    </p>
                                </div>
                            )}
