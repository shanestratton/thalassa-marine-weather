        ...
        return dataset.windSpeeds.map((tws, wIdx) => {
            ...
            const pathPort =
                mirrorPoints.length > 0
                    ? `M ${mirrorPoints[0][0]},${mirrorPoints[0][1]} ` +
                      mirrorPoints
                          .slice(1)
                          .map((p) => `L ${p[0]},${p[1]}`)
                          .join(' ')
                ...
                <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-1 px-2">
                        ...
                        data.windSpeeds.map((tws, i) => (
                            <div key={`f-${tws}`} className="flex items-center gap-1.5">
                                <div
                                    ...
                                    style={{ backgroundColor: WIND_COLORS[i % WIND_COLORS.length] }}
                                />
                    ...
                    {hasOverlay && (
                        <div className="flex items-center gap-1.5 ml-2 pl-2 border-l border-white/10">
                            <div
                                className="w-4 h-0.5 rounded-full bg-emerald-400"
                                style={{
                                    backgroundImage:
                                        'repeating-linear-gradient(90deg, #4ade80 0 4px, transparent 4px 7px)',
                                }}
                            />
