...
import { FlagIcon, PhoneIcon, RadioTowerIcon, AlertTriangleIcon, ShareIcon, MapPinIcon } from '../Icons';

/* ═══════════════════════════════════════════════════════════════
   COUNTRY CLEARANCE DATABASE
   ═══════════════════════════════════════════════════════════════ */

interface ClearanceContact {
    name: string;
    phone?: string;
    email?: string;
    vhf?: string;
...
const COUNTRY_DB: Record<string, CountryClearance> = {
    australia: {
        country: 'Australia',
        flag: '🇦🇺',
        difficulty: 'complex',
...
function findCountryData(country: string | undefined): CountryClearance | undefined {
            ...
            {activeData ? (
                <div className="animate-in fade-in duration-200 space-y-4">
                    ...
                    <div className={`flex items-center justify-between px-4 py-2.5 rounded-xl border ${difficultyStyle[activeData.difficulty].bg} ${difficultyStyle[activeData.difficulty].border}`}>
                        ...
                        {activeData.fees && (
                            <span className="text-[10px] text-gray-400 font-mono">{activeData.fees}</span>
                        )}
                    </div>
                        ...
                        <div className="space-y-2">
                            {(activeTab === 'depart' ? activeData.departureProcedure : activeData.arrivalProcedure).map((step, i) => (
                                <div key={i} className="flex items-start gap-3 group">
                                    <span className="w-5 h-5 rounded-full bg-sky-500/20 border border-sky-500/30 flex items-center justify-center text-[10px] font-black text-sky-400 shrink-0 mt-0.5 group-hover:bg-sky-500/30 transition-colors">
                                        {i + 1}
                                    </span>
                            ...
                            {activeData.requiredDocuments.map((doc, i) => (
                                ...
                                >
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        {doc.notes && (
                                            <div className="text-[10px] text-gray-500 mt-0.5">{doc.notes}</div>
                                        )}
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="space-y-2">
                            {activeData.contacts.map((contact, i) => (
                                <div key={i} className="bg-white/5 border border-white/5 rounded-xl px-4 py-3">
                                    ...
                                    {contact.notes && (
                                        <div className="text-[10px] text-gray-500 mt-1">{contact.notes}</div>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {activeData.portsOfEntry.map((port, i) => (
                                <span
                                    key={i}
                                    className="text-[10px] text-gray-300 bg-white/5 border border-white/5 rounded-full px-2.5 py-1 hover:bg-sky-500/10 hover:border-sky-500/20 hover:text-sky-200 transition-colors cursor-default"
                                >
                                    {port}
                                </span>
                            ))}
