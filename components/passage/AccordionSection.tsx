...
export const AccordionSection: React.FC<AccordionSectionProps> = React.memo(
    ({ title, subtitle, icon, accent = 'sky', badge, defaultOpen = false, children, className = '' }) => {
        ...
        return (
            <div
                className={`bg-slate-900/80 border border-white/10 rounded-2xl overflow-hidden shadow-xl relative ${className}`}
            >
                {/* Ambient glow */}
                <div
                ...
                >
                    {/* Icon */}
                    <div
                        className={`p-2 rounded-xl bg-white/5 ${colors.icon} shrink-0 group-hover:scale-105 transition-transform`}
                    >
                        {icon}
                    </div>
                    ...
                    {badge && !isOpen && (
                        <div
                            className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${colors.badge} shrink-0 animate-in fade-in duration-200`}
                        >
                            {badge}
                        </div>
                    )}
                ...
                >
                    <div ref={contentRef} className="px-5 pb-5">
                        {/* Separator */}
                        <div
                            className={`h-px ${isOpen ? 'bg-white/5' : 'bg-transparent'} mb-4 -mx-5 transition-colors`}
                        />
                        {children}
                    </div>
                </div>
