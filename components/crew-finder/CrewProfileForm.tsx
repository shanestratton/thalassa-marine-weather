...
const CrewProfileFormInner: React.FC<CrewProfileFormProps> = ({
    state, dispatch,
    onSaveProfile, onPhotoUpload, onPhotoRemove, onDeleteProfile,
    myProfileScrollRef, fileInputRef,
}) => {
    // ── Destructure state ──
    const {
        editListingType, editFirstName, editGender, editAge, editBio,
        editRegion, editExperience, editSkills, editAvailFrom, editAvailTo,
        ...
        editHasPartner, editPartnerDetails, editPhotos, uploadingPhotoIdx,
        photoError, pendingPhotoIdx, kbHeight, saving, saved,
        showPreview, showDeleteConfirm, deleting, profile,
    } = state;

    // ── Inline setters (dispatch wrappers) ──
    const set = useCallback(<T,>(type: CrewFinderAction['type'], payload: T) =>
        dispatch({ type, payload } as CrewFinderAction), [dispatch]);
    ...
    const toggleEditSkill = (skill: string) =>
        dispatch({ type: 'SET_EDIT_SKILLS', payload: editSkills.includes(skill) ? editSkills.filter(s => s !== skill) : [...editSkills, skill] });
    const toggleVibe = (v: string) =>
        dispatch({ type: 'SET_EDIT_VIBE', payload: editVibe.includes(v) ? editVibe.filter(x => x !== v) : [...editVibe, v] });
    const toggleLanguage = (lang: string) =>
        dispatch({ type: 'SET_EDIT_LANGUAGES', payload: editLanguages.includes(lang) ? editLanguages.filter(l => l !== lang) : [...editLanguages, lang] });
    const toggleInterest = (interest: string) =>
        dispatch({ type: 'SET_EDIT_INTERESTS', payload: editInterests.includes(interest) ? editInterests.filter(i => i !== interest) : [...editInterests, interest] });
    ...
    return (
        ...
        <div ref={myProfileScrollRef as any} className="flex-1 overflow-y-auto overscroll-contain px-5 py-6 space-y-5" style={{ paddingBottom: kbHeight > 0 ? `${kbHeight}px` : '1rem', WebkitOverflowScrolling: 'touch' as any }}>
            ...
            <div>
                ...
                <div className="flex gap-2 flex-wrap">
                    {AGE_RANGES.map(age => (
                        <button
                            key={age}
                            onClick={() => { setEditAge(editAge === age ? '' : age); }}
