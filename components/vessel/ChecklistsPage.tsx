    ...
    return (
        <div className="relative overflow-hidden rounded-xl">
            ...
            <div
                ...
                ref={ref}
                onClick={() => { if (swipeOffset === 0) onEdit(); }}
            >
...
export const ChecklistsPage: React.FC<ChecklistsPageProps> = ({ onBack }) => {
    ...
    const grouped = headings.map(h => ({
        heading: h,
        items: entries.filter(e => e.type === 'detail' && e.heading_id === h.id)
    ...
    const filtered = searchQuery.trim()
        ? grouped.map(g => ({
            ...
            g.heading.text.toLowerCase().includes(searchQuery.toLowerCase()) || g.items.length > 0
        )
        : grouped;
    ...
    const toggleRunItem = useCallback((entryId: string) => {
        ...
        setRunItems(prev => prev.map(item => {
            ...
            const next: RunItemStatus =
                item.status === 'unchecked' ? 'pass' :
                    item.status === 'pass' ? 'fail' : 'unchecked';
    ...
    const runGrouped = runItems.reduce<{ heading: string; items: ChecklistRunItem[] }[]>((acc, item) => {
        ...
        else { acc.push({ heading: item.heading, items: [item] }); }
        return acc;
    }, []);
    ...
    return (
        <div className="relative h-full bg-slate-950 overflow-hidden">
            <div className="flex flex-col h-full">
                ...
                <PageHeader
                    ...
                    subtitle={
                        <p className="text-label text-gray-500 font-bold uppercase tracking-widest">
                            {headings.length} Section{headings.length !== 1 ? 's' : ''} · {totalDetails} Item{totalDetails !== 1 ? 's' : ''}
                        </p>
                    }
                    action={
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-4 min-h-0 space-y-4">
                    {loading ? (
                        <div className="space-y-3">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4 flex items-center gap-3">
                        ...
                        <EmptyState
                            ...
                            subtitle={searchQuery ? 'Try a different search term.' : 'Slide below to add your first section or item.'}
                            className="py-16"
                        />
                    ) : (
                        filtered.map(group => (
                ...
                {/* ═══ ADD / EDIT MODAL ═══ */}
                {showForm && (
                    <ModalSheet isOpen={true} onClose={() => { setShowForm(false); resetForm(); }} title={editEntry ? 'Edit Item' : 'Add to Checklist'} alignTop>

                        {/* Type toggle — Heading or Detail */}
                        {!editEntry && (
                            <div className="mb-4">
                                <label className="text-label text-gray-500 font-bold uppercase tracking-widest block mb-2">Type</label>
                                ...
                                {headings.length === 0 && formType === 'heading' && (
                                    <p className="text-[10px] text-amber-400/80 mt-2 text-center">Add a heading first, then you can add detail items</p>
                                )}
                            </div>
                        )}
                        ...
                        {formType === 'detail' && headings.length > 0 && (
                            <div className="mb-3">
                                <label className="text-label text-gray-500 font-bold uppercase tracking-widest block mb-1.5">Under Section</label>
                                <select
                                    value={formHeadingId}
                                ...
                                >
                                    {headings.map(h => (
                                        <option key={h.id} value={h.id} className="bg-slate-800">{h.text}</option>
                                    ))}
                                </select>
                            </div>
                        )}
                        ...
                        <div className="mb-4">
                            <FormField
                                ...
                                placeholder={formType === 'heading' ? 'Pre-Start, Shut Down, Navigation...' : 'Check oil level, test bilge pump...'}
                                required
                            />
                        </div>
                ...
                {showRun && (
                    <div className="fixed inset-0 z-[999] bg-slate-950 flex flex-col" style={{ paddingTop: 'max(1rem, env(safe-area-inset-top))' }}>

                        {/* Run header */}
                        <div className="shrink-0 px-4 pb-3">
                            <div className="flex items-center justify-between mb-3">
                                <button
                                    onClick={() => { setShowRun(false); setRunItems([]); }}
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3">
                                    {runPassCount > 0 && (
                                        <span className="text-[10px] text-emerald-400 font-bold">✓ {runPassCount}</span>
                                    )}
                                    {runFailCount > 0 && (
                                        <span className="text-[10px] text-red-400 font-bold">✗ {runFailCount}</span>
                                    )}
                                </div>
                            </div>
                        </div>
                            ...
                            {runGrouped.map(group => (
                                <div key={group.heading}>
                                    ...
                                    <div className="flex items-center gap-2 mb-2.5 sticky top-0 bg-slate-950 py-1 z-10">
                                        ...
                                        <span className="text-[11px] font-black text-emerald-400 uppercase tracking-[0.2em]">{group.heading}</span>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        {group.items.map(item => {
                                            ...
                                            return (
                                                <div key={item.entry_id} className={`rounded-xl border ${styles.border} ${styles.bg} overflow-hidden transition-all duration-200`}>
                                                    <div
                                                    ...
                                                    >
                                                        ...
                                                        {/* Label */}
                                                        <span className={`text-sm font-medium flex-1 ${styles.text} ${item.status === 'pass' ? 'line-through opacity-60' : ''
                                                            }`}>
                                                            {item.text}
                                                        </span>
                        ...
                        <div className="shrink-0 px-4 pt-3 bg-slate-950" style={{ paddingBottom: 'calc(5rem + env(safe-area-inset-bottom))' }}>
                            ...
                            >
                                {runCheckedCount === runTotal
                                    ? (runFailCount > 0 ? `Complete — ${runFailCount} Failed` : '✅ All Passed — Complete')
