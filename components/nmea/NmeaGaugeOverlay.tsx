...
export const NmeaGaugeOverlay: React.FC<NmeaGaugeOverlayProps> = ({
    metricId,
    metric,
    onClose
}) => {
    ...
    const gaugeConfig: Record<GaugeMetricId, GaugeConfig> = {
        cog: {
            ...
            component: <CompassGauge value={metric.value} label="COG" accentColor="#22d3ee" freshness={metric.freshness} />
        },
        heading: {
            title: 'Heading',
            component: <CompassGauge value={metric.value} label="HDG" accentColor="#f59e0b" freshness={metric.freshness} />
        },
        tws: {
            title: 'True Wind Speed',
            component: <ArcGauge value={metric.value} min={0} max={60} unit="kts" label="True Wind Speed" accentColor="#38bdf8"
                zones={[
                ...
                majorTick={10} freshness={metric.freshness} />
        },
        twa: {
            title: 'True Wind Angle',
            component: <ArcGauge value={metric.value} min={0} max={180} unit="°" label="True Wind Angle" decimals={0}
                accentColor="#a78bfa"
                zones={[
                ...
                majorTick={30} freshness={metric.freshness} />
        },
        stw: {
            title: 'Speed Through Water',
            component: <ArcGauge value={metric.value} min={0} max={15} unit="kts" label="Speed Through Water"
                accentColor="#06b6d4"
                zones={[
                ...
                majorTick={5} freshness={metric.freshness} />
        },
        sog: {
            title: 'Speed Over Ground',
            component: <ArcGauge value={metric.value} min={0} max={15} unit="kts" label="Speed Over Ground"
                accentColor="#34d399"
                zones={[
                    { from: 0, to: 5, color: '#38bdf8' },
                    { from: 5, to: 10, color: '#22c55e' },
        ...
        waterTemp: {
            ...
            component: <ArcGauge value={metric.value} min={0} max={40} unit="°C" label="Water Temperature"
                accentColor="#06b6d4"
                zones={[
        ...
        voltage: {
            ...
            component: <BarGauge value={metric.value} min={10} max={15} unit="V" label="Battery Voltage"
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex items-center justify-center px-4 pb-safe-bottom"
                style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 20px) + 20px)' }}
            >
                <div className={`w-full max-w-md transform transition-all duration-500 ${visible ? 'scale-100 translate-y-0' : 'scale-90 translate-y-8'
                    }`}>
                    {config.component}
                </div>
            </div>
            ...
            {/* Freshness indicator */}
            <div className="absolute bottom-6 left-0 right-0 flex justify-center" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
                <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${metric.freshness === 'live' ? 'bg-emerald-400 animate-pulse' :
                        metric.freshness === 'stale' ? 'bg-amber-400' :
                            'bg-red-400'
                        }`} />
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">
                        {metric.freshness === 'live' ? 'Live Data' :
                            metric.freshness === 'stale' ? 'Stale Data' :
                                'No Data'}
                    </span>
                </div>
            </div>
