...
export const DepthGauge: React.FC<DepthGaugeProps> = ({
    ...
    // Bar dimensions
    const BAR_X = 100, BAR_Y = 30, BAR_W = 60, BAR_H = 240;
        ...
        <div className="flex flex-col items-center justify-center w-full h-full gap-6">
            <div className="relative" style={{ width: '75vw', maxWidth: '320px', aspectRatio: '0.8' }}>
                <svg viewBox="0 0 260 310" className="w-full h-full">
                    ...
                    <rect x={BAR_X} y={BAR_Y} width={BAR_W} height={BAR_H} rx="8" ry="8"
                        fill="rgba(15,23,42,0.6)" stroke="rgba(255,255,255,0.1)" strokeWidth="1"
                        opacity={opacity}
                    />
                    ...
                    {value !== null && fraction > 0.01 && (
                        <g opacity={opacity}>
                            ...
                            {/* Water surface line */}
                            <line
                                x1={BAR_X + 4} y1={BAR_Y + BAR_H - fillH}
                                x2={BAR_X + BAR_W - 4} y2={BAR_Y + BAR_H - fillH}
                    ...
                    {/* Glass overlay */}
                    <rect x={BAR_X} y={BAR_Y} width={BAR_W} height={BAR_H} rx="8" ry="8"
                        fill="url(#depth-glass)" opacity={opacity * 0.5} />
                    ...
                    <g opacity={opacity}>
                        {markers.map(d => {
                            ...
                            return (
                                <g key={d}>
                                    <line
                                        x1={BAR_X - (isMajor ? 12 : 6)} y1={yPos}
                                        x2={BAR_X - 2} y2={yPos}
                                        stroke={isMajor ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.15)'}
                                        strokeWidth={isMajor ? 1.5 : 0.8}
                                    />
                                    {isMajor && (
                                        <text x={BAR_X - 18} y={yPos}
                                            textAnchor="end" dominantBaseline="central"
                                            fill="rgba(148,163,184,0.7)" fontSize="11" fontWeight="600"
                    ...
                    {value !== null && (
                        <g opacity={opacity} style={{ transition: 'transform 0.6s ease' }}>
                            ...
                            />
                            <rect
                                x={BAR_X + BAR_W + 14} y={BAR_Y + fraction * BAR_H - 14}
                                width="52" height="28" rx="6"
                            ...
                            />
                            <text
                                x={BAR_X + BAR_W + 40} y={BAR_Y + fraction * BAR_H}
                    ...
                    <text x={BAR_X + BAR_W / 2} y={BAR_Y - 10}
                        textAnchor="middle" fill="rgba(148,163,184,0.4)" fontSize="9"
                        fontWeight="700" fontFamily="system-ui" letterSpacing="0.15em"
                        opacity={opacity}
                    >
                        SURFACE
                    </text>
                    ...
                    {effectiveMax > 5 && (
                        <rect
                            x={BAR_X + 1} y={BAR_Y + 1}
                            width={BAR_W - 2} height={(5 / effectiveMax) * BAR_H}
                            fill="rgba(239,68,68,0.06)" rx="7" ry="0"
                            opacity={opacity}
                        />
                    )}
                </svg>
            ...
            <div className="flex flex-col items-center gap-1">
                <div className="flex items-baseline gap-1.5" style={{ opacity }}>
                    <span className={`text-6xl font-black tracking-tighter font-mono tabular-nums ${isShallow ? 'text-red-400' : 'text-white'}`}>
