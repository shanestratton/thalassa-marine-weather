...
export const CrewManagement: React.FC<CrewManagementProps> = React.memo(({ onBack }) => {
    ...
    const reloadDropdown = useCallback(async () => {
        ...
        if (visibleCachedDrafts.length > 0) {
            ...
            // routes appear immediately. The fast path must paint the SAME
            // shape the settled pass produces — pruned to one row per route,
            // "(Passage)" rollups included — or every stale replan leftover
            // flashes until the network pass lands (Shane 2026-08-27: "ghost
            // legs everywhere, but they eventually go away").
            setDraftVoyages((previous) => {
                const prior = new Map(previous.map((row) => [row.id, row] as const));
                const own = pruneToBestRowPerRoute(
                    visibleCachedDrafts.map((row) => preserveRouteGeometry(row, prior.get(row.id))),
                    cachedCanonicalTraces,
        ...
        setDraftVoyages((previous) => {
            const prior = new Map(previous.map((row) => [row.id, row] as const));
            // Same ghost-proofing as the cached paint above: prune + rollups
            // on every frame, not just the settled one.
            const own = pruneToBestRowPerRoute(
        ...
        // ── ONE row per saved route + derived "(Passage)" rollups ──
        // Shared helpers (pruneToBestRowPerRoute / tripPassageRollupRows, at
        // module scope above) — the SAME machinery runs on the cached fast
        // paints so no ghost rows flash before this settled pass lands. The
        // prune sees canonicalById's values (traces linked or adopted during
        // THIS pass included), the rollups the synced canonical set.
        {
            const pruned = pruneToBestRowPerRoute(ownRows, [...canonicalById.values()], selectedPassageRef.current);
            ownRows.length = 0;
            ownRows.push(...pruned);
        }
