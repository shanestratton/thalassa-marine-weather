...
export const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
    ...
    const showConfirm = (title: string, message: string, confirmLabel: string, destructive: boolean, onConfirm: () => Promise<void>) => {
        ...
        <div className="flex flex-col h-full" role="region" aria-label="Admin Panel">
            ...
            <div className="sticky top-0 z-10 bg-slate-900/95 border-b border-white/[0.06]">
                <div className="flex items-center gap-3 px-4 py-3">
                    <button onClick={onClose} aria-label="Back to channels" className="w-10 h-10 flex items-center justify-center rounded-xl bg-white/[0.04] hover:bg-white/[0.08] transition-colors">
                            ...
                            <div className="px-4 pb-2">
                                ...
                                <div className="space-y-2">
                                    {joinRequests.map(req => (
                                        <div key={req.id} className="rounded-xl border border-purple-500/15 bg-purple-500/[0.03] p-3 space-y-2.5">
                                            <div className="flex items-center gap-2.5">
                                                {req.avatar_url ? (
                                                    <img src={req.avatar_url} className="w-9 h-9 rounded-full object-cover border border-white/10" alt={`${req.display_name} avatar`} />
                                                ) : (
                                            ...
                                            <div className="flex gap-1.5">
                                                <button onClick={() => handleRejectJoinRequest(req.id)} aria-label={`Reject ${req.display_name}'s request`} className="flex-1 py-2.5 rounded-xl bg-red-500/10 border border-red-500/15 text-[11px] font-bold text-red-400 active:scale-95 min-h-[44px]">❌ Reject</button>
                                                <button onClick={() => handleApproveJoinRequest(req.id)} aria-label={`Approve ${req.display_name}'s request`} className="flex-1 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/15 text-[11px] font-bold text-emerald-400 active:scale-95 min-h-[44px]">✅ Approve</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="px-4 space-y-2">
                            {loading ? (
                                ...
                                <div className="text-center py-12 space-y-3">
                                    ...
                                    <p className="text-xs text-white/30">{search ? `No one matching "${search}"` : 'No users to display'}</p>
                                </div>
                            ) : (
                                filteredUsers.map(user => {
                                    ...
                                    return (
                                        <div key={user.user_id} className={`rounded-xl border overflow-hidden transition-all ${user.is_blocked ? 'border-red-500/20 bg-red-500/5' : 'border-white/[0.06] bg-white/[0.02]'}`}>
                                            ...
                                            >
                                                {user.avatar_url ? (
                                                    <img src={user.avatar_url} className="w-10 h-10 rounded-full object-cover border border-white/10" alt="" />
                                                ) : (
                                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-sky-600 to-indigo-600 flex items-center justify-center text-sm font-bold text-white">{user.display_name[0].toUpperCase()}</div>
                                                )}
                                                <div className="flex-1 min-w-0">
                                            ...
                                            {isExpanded && !isMe && (
                                                <div className="px-3.5 pb-3.5 space-y-2 border-t border-white/[0.04] pt-2.5">
                                                    ...
                                                    <div className="flex gap-1.5">
                                                        {(['admin', 'moderator', 'member'] as ChatRole[]).map(r => (
                                                            <button
                                                                ...
                                                                className={`flex-1 py-2.5 rounded-xl border text-[11px] font-bold uppercase tracking-wider transition-all active:scale-95 min-h-[44px] ${user.role === r ? ROLE_STYLES[r].bg + ' ' + ROLE_STYLES[r].text : 'bg-white/[0.04] border-white/[0.06] text-white/50'
                                                                    }`}
                                                            >
                                                                {ROLE_STYLES[r].label}
                                                            </button>
                                                    ...
                                                    {isMuted ? (
                                                        <button onClick={() => handleUnmute(user.user_id)} aria-label="Unmute user" className="w-full py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold text-emerald-400 uppercase tracking-wider active:scale-95 min-h-[44px]">🔊 Unmute User</button>
                                                    ) : (
                                                        <div className="flex gap-1.5">
                                                            <input value={muteHours} onChange={e => setMuteHours(e.target.value.replace(/\D/g, ''))} placeholder="Hours" inputMode="numeric" aria-label="Mute duration in hours" className="w-20 px-2.5 py-3 rounded-xl bg-white/[0.06] border border-white/10 text-xs text-white placeholder-white/30 outline-none min-h-[44px]" />
                                                            <button onClick={() => handleMute(user.user_id)} aria-label={`Mute for ${muteHours || '?'} hours`} className="flex-1 py-3 rounded-xl bg-orange-500/10 border border-orange-500/20 text-[11px] font-bold text-orange-400 uppercase tracking-wider active:scale-95 min-h-[44px]">🔇 Mute {muteHours || '?'} hrs</button>
                                                        </div>
                                                    )}
                                                    {/* Block/Unblock — 44px min */}
                                                    {user.is_blocked ? (
                                                        <button onClick={() => handleUnblock(user.user_id)} aria-label="Unblock user" className="w-full py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold text-emerald-400 uppercase tracking-wider active:scale-95 min-h-[44px]">✅ Unblock User</button>
                                                    ) : (
                                                        <button onClick={() => handleBlock(user.user_id)} aria-label="Block user permanently" className="w-full py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-[11px] font-bold text-red-400 uppercase tracking-wider active:scale-95 min-h-[44px]">🚫 Block Permanently</button>
                                                    )}
                                                </div>
                                            )}
                                            {isExpanded && isMe && (
                        ...
                        <div>
                            ...
                            {pendingChannels.length === 0 ? (
                                <div className="text-center py-8 space-y-2">
                                    ...
                                    <p className="text-xs text-white/25">Channel proposals from users will appear here</p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    {pendingChannels.map(ch => (
                                        <div key={ch.id} className="rounded-xl border border-sky-500/15 bg-sky-500/[0.03] p-3.5 space-y-2.5">
                                            <div className="flex items-center gap-2.5">
                                            ...
                                            <div className="flex gap-1.5">
                                                <button onClick={() => handleRejectChannel(ch.id)} aria-label={`Reject ${ch.name}`} className="flex-1 py-2.5 rounded-xl bg-red-500/10 border border-red-500/15 text-[11px] font-bold text-red-400 active:scale-95 min-h-[44px]">❌ Reject</button>
                                                <button onClick={() => handleApproveChannel(ch.id)} aria-label={`Approve ${ch.name}`} className="flex-1 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/15 text-[11px] font-bold text-emerald-400 active:scale-95 min-h-[44px]">✅ Approve</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {activeChannels.length === 0 ? (
                                ...
                                <div className="space-y-1.5">
                                    {activeChannels.map(ch => (
                                        <div key={ch.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-3.5 py-3 flex items-center gap-2.5 min-h-[52px]">
                    ...
                    <div className="px-4 pt-4 space-y-2">
                        ...
                        {auditLog.length === 0 ? (
                            <div className="text-center py-16 space-y-3">
                                ...
                                <p className="text-xs text-white/25">Admin actions will be logged here for accountability.<br />Every role change, block, mute, and channel action is tracked.</p>
                            </div>
                        ) : (
                            auditLog.map((entry, i) => {
                                ...
                                return (
                                    <div key={entry.id || i} className="rounded-xl border border-white/[0.04] bg-white/[0.015] px-3.5 py-3 flex items-start gap-2.5">
