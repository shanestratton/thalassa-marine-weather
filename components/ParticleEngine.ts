...
export class ParticleEngine {
    ...
    constructor(canvas: HTMLCanvasElement, map: any, sampleVal: any, sampleDir: any) {
        ...
        if (isLowPower) {
            this.particleCount = 300; // Minimal density for older phones
            this.fpsInterval = 1000 / 20; // 20 FPS throttle
        } else if (isHighRes) {
            this.particleCount = 450; // Balance for retina screens
        }
    ...
    public setLayer(layer: 'wind' | 'waves' | 'rain') {
    ...
    private getParticleColor(val: number): string {
        const a = 0.55; // Base Alpha for smoother blending
        if (this.activeLayer === 'wind') {
            if (val < 3) return `rgba(150, 150, 150, 0.2)`; 
            if (val < 8) return `rgba(59, 130, 246, ${a})`;   // blue-500
            if (val < 15) return `rgba(34, 197, 94, ${a})`;  // green-500
            if (val < 25) return `rgba(234, 179, 8, ${a})`;  // yellow-500
            if (val < 35) return `rgba(249, 115, 22, ${a})`;  // orange-500
            if (val < 45) return `rgba(239, 68, 68, ${a})`;  // red-500
            return `rgba(217, 70, 239, ${a})`;                // fuchsia-500
        ...
        try {
            ...
            this.particles.forEach(p => {
                ...
                if (this.activeLayer === 'rain') {
                    ...
                    this.ctx.beginPath();
                    this.ctx.strokeStyle = `rgba(186, 230, 253, ${Math.min(val * 0.2, 0.8)})`;
