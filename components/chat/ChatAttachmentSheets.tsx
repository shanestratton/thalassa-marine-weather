...
export interface PinDropSheetProps {
    ...
    /** Which receiver answered — "Boat GPS", "USB GPS (Pi)", "Phone GPS". */
    pinRungLabel: string | null;
...
export const PinDropSheet: React.FC<PinDropSheetProps> = React.memo(
    ({
        ...
        // A CENTRED MODAL, not an inline bottom sheet.
        // This lived inside ChatPage's flex column, which the keyboard
        // guard resizes — so raising the keyboard for the caption field
        // shrank the very sheet holding it, and the input it was trying
        // to reveal went with it (Shane 2026-09-05: "endless problems
        // with the keyboard for the send my location").
        //
        // Centred and portalled, it is outside that column entirely:
        // nothing resizes it, the keyboard simply overlays, and the
        // standing rule holds — every modal centred, clear of the tab
        // bar, scrolling internally.
        return (
            <OverlayPortal className="flex items-center justify-center p-4" role="presentation" onClick={onClose}>
                ...
                <section
                    ...
                    className="relative flex max-h-[80vh] w-full max-w-md flex-col overflow-y-auto overscroll-contain rounded-3xl border border-emerald-400/20 bg-slate-900 px-4 py-4 shadow-[0_20px_60px_rgba(0,0,0,0.6)]"
                >
                    ...
                    {pinLoading ? (
                        ...
                        <>
                            <div className="flex items-center justify-between gap-3 rounded-xl border border-emerald-400/20 bg-emerald-400/6 px-3 py-2.5 mb-2">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        {/* WHICH receiver, not just "GPS". The
                                            boat's Garmin, the Pi's USB stick
                                            and the phone in your pocket can be
                                            kilometres apart, and a shared
                                            position that does not say which one
                                            answered is a position you cannot
                                            check. */}
                            ...
                            <div className="relative w-full h-[136px] rounded-2xl overflow-hidden border border-white/10 mb-2">
                                ...
                                />
                                {/* NO OVERLAY PIN HERE.
                                    getStaticMapUrl already bakes a marker into
                                    the image — `pin-l+ff4466(lng,lat)` on the
                                    Mapbox URL, `markers=…,ol-marker` on the OSM
                                    fallback. Drawing a second one centred on
                                    the same coordinate put two pins one on top
                                    of the other (Shane 2026-09-05). The map's
                                    own marker wins: it is positioned by the
...
export const PoiPickerSheet: React.FC<PoiPickerSheetProps> = React.memo(
    ({
        ...
        // A CENTRED MODAL, for the same reason as the location sheet
        // above. An internally-scrolling bottom sheet is exactly the case
        // the app-wide keyboard guard cannot rescue — the class comment in
        // index.css says so — and the search field here is the one a
        // skipper types into. Portalled out of the resized column, that
        // whole class of problem stops applying.
        return (
            <OverlayPortal className="flex items-center justify-center p-4" role="presentation" onClick={onClose}>
                <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
                <section
                    ...
                    className="relative flex max-h-[80vh] w-full max-w-md flex-col overflow-y-auto overscroll-contain rounded-3xl border border-sky-400/20 bg-slate-900 px-4 py-4 shadow-[0_20px_60px_rgba(0,0,0,0.6)]"
                >
                    <div className="flex items-start justify-between gap-4 mb-3">
                        <div>
