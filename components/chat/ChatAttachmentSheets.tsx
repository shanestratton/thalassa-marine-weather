            ...
            {reportSent ? (
                ...
                <>
                    ...
                    <div className="flex gap-2">
                        <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-white/[0.03] text-xs text-white/60 hover:bg-white/[0.06] transition-colors">Cancel</button>
                        <button onClick={onSubmit} className="flex-1 py-2.5 rounded-xl bg-amber-500/15 text-xs text-amber-400 font-medium hover:bg-amber-500/25 transition-colors">Submit Report</button>
                    </div>
                </>
            )}
        </div>
...
export const PinDropSheet: React.FC<PinDropSheetProps> = React.memo(({
    pinLat, pinLng, pinCaption, setPinCaption, setPinLat, setPinLng, pinLoading, savedPins, onSendPin, onClose,
}) => (
    <div className="flex-shrink-0 border-t border-white/[0.06] bg-slate-900 px-4 py-3">
        <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={onClose} className="text-white/60 hover:text-white/60 text-lg transition-colors px-2">✕</button>
        </div>
        {pinLoading ? (
            ...
            <>
                    ...
                    <div className="mb-2">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1" style={{ scrollbarWidth: 'none' }}>
                            {savedPins.map(sp => (
                                <button
                                    key={sp.id}
                                    onClick={() => { setPinLat(sp.latitude); setPinLng(sp.longitude); setPinCaption(sp.caption); }}
                ...
                <div className="w-full h-[120px] rounded-xl overflow-hidden border border-white/[0.08] mb-2">
                    <img src={getStaticMapUrl(pinLat, pinLng)} alt="Pin location" className="w-full h-full object-cover" loading="eager" />
                </div>
                ...
                <div className="flex items-center gap-2">
                    <input type="text" value={pinCaption} onChange={e => setPinCaption(e.target.value)} onKeyDown={e => e.key === 'Enter' && onSendPin()}
                        placeholder="What's here? (e.g. Great anchorage)" className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-sky-500/30 transition-colors" maxLength={120} />
                    <button onClick={onSendPin} className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-sky-500/20 to-sky-500/20 hover:from-sky-500/30 hover:to-sky-500/30 text-sm text-white/80 font-bold transition-all active:scale-95 whitespace-nowrap">📍 Drop</button>
                </div>
            </>
        )}
    </div>
...
export const PoiPickerSheet: React.FC<PoiPickerSheetProps> = React.memo(({
    ...
    <div className="flex-shrink-0 border-t border-white/[0.06] bg-slate-900 px-4 py-3">
        ...
        {pinLoading ? (
            ...
            <>
                <div ref={poiMapRef as React.RefObject<HTMLDivElement>} className="w-full h-[200px] rounded-xl overflow-hidden border border-white/[0.08] mb-2" />
                <p className="text-[11px] text-white/25 mb-2 text-center tabular-nums">
                    📍 {Math.abs(pinLat).toFixed(4)}°{pinLat < 0 ? 'S' : 'N'}, {Math.abs(pinLng).toFixed(4)}°{pinLng < 0 ? 'W' : 'E'}
                    <span className="ml-2 text-white/15">• Tap or drag to set location</span>
                </p>
                <div className="flex items-center gap-2">
                    <input type="text" value={pinCaption} onChange={e => setPinCaption(e.target.value)} onKeyDown={e => e.key === 'Enter' && onSendPoi()}
                        placeholder="Describe this spot..." className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-sky-500/30 transition-colors" maxLength={120} />
                    <button onClick={onSendPoi} className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500/20 to-emerald-500/20 hover:from-emerald-500/30 hover:to-emerald-500/30 text-sm text-white/80 font-bold transition-all active:scale-95 whitespace-nowrap">🗺️ Share</button>
                </div>
            </>
        )}
    </div>
...
export interface TrackPickerSheetProps {
    voyageList: { voyageId: string; entryCount: number; distance: number; startTime: string; endTime: string; entries: ShipLogEntry[] }[];
        ...
        {trackLoadingVoyages ? (
            ...
            <div className="space-y-2 overflow-y-auto max-h-[240px] pb-1" style={{ scrollbarWidth: 'thin' }}>
                {voyageList.map(v => {
                    ...
                    return (
                        <div key={v.voyageId} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] transition-all">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[11px] text-white/60 tabular-nums">{v.distance}nm · {v.entryCount} pts · {durationHrs}h</p>
                            </div>
                            <button onClick={() => onSendTrack(v)} disabled={trackSharing}
                                ...
                                {trackSharing ? <div className="w-4 h-4 border-2 border-emerald-500/30 rounded-full border-t-teal-500 animate-spin" /> : '⛵ Share'}
                            </button>
                        </div>
                    );
                })}
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 p-6" onClick={onClose}>
        <div className="w-full max-w-sm bg-slate-900/95 border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 pb-5 flex gap-2 pt-2">
                <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-white/[0.05] border border-white/[0.08] text-white/60 text-sm font-bold transition-all active:scale-95">Cancel</button>
                <button onClick={() => onImport(track.trackId, track.title)} className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-sky-600 to-sky-600 text-white text-sm font-bold transition-all active:scale-95 shadow-lg shadow-sky-500/20">⬇ Import Track</button>
            </div>
        </div>
    </div>
));
