...
export const VoyageResults: React.FC<VoyageResultsProps> = ({
    ...
    return (
        <div className="max-w-7xl mx-auto w-full animate-in fade-in slide-in-from-bottom-8 duration-700 pb-12 flex-1 flex flex-col">
            ...
            {/* TOP SUMMARY STACK - REDESIGNED */}
            <div className="flex flex-col gap-6 mb-8">

                {/* 1. VOYAGE OVERVIEW CARD (Full Width) */}
                <div className="w-full bg-[#0f172a] border border-white/10 rounded-3xl p-0 relative overflow-hidden shadow-2xl flex flex-col">
                    ...
                    {/* Stats */}
                    {/* Voyage Stats Grid */}
                    <div className="px-6 md:px-8 pb-8 grid grid-cols-2 lg:grid-cols-4 gap-3 relative z-10">
                        {/* Distance */}
                        <div className="bg-white/5 rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:bg-white/10 transition-colors min-h-[110px]">
                            ...
                            <div>
                                <span className="text-base md:text-lg font-bold text-white block">{voyagePlan.distanceApprox}</span>
                                <span className="text-[10px] text-gray-500">Great Circle</span>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            className="bg-gradient-to-br from-sky-500/20 to-blue-600/20 rounded-2xl p-4 border border-sky-500/30 flex flex-col justify-center items-center group hover:from-sky-500/30 hover:to-blue-600/30 transition-all cursor-pointer min-h-[110px] col-span-2 lg:col-span-1"
                        >
                            <ShareIcon className="w-6 h-6 text-sky-400 mb-2 group-hover:scale-110 transition-transform" />
                            <span className="text-xs font-bold text-sky-300 uppercase tracking-widest">Export PDF</span>
                            <span className="text-[9px] text-sky-400/70 mt-1">Printable Brief</span>
                        </div>
                        ...
                        <div className="bg-white/5 rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:bg-white/10 transition-colors min-h-[110px]">
                            ...
                            <div>
                                <span className="text-base md:text-lg font-bold text-white block leading-tight">{voyagePlan.durationApprox}</span>
                        ...
                        <div className="bg-white/5 rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:bg-white/10 transition-colors min-h-[110px]">
                            ...
                            <div>
                                <div className="flex items-baseline gap-1">
                                    <span className="text-base md:text-lg font-bold text-white">{voyagePlan.suitability?.maxWindEncountered ?? '--'}</span>
                        ...
                        {/* Viability Status - Fixed Truncation */}
                        <div className={`rounded-2xl p-4 border flex flex-col justify-between min-h-[110px] ${getStatusClasses(voyagePlan.suitability?.status)}`}>
                            <div className="flex items-center gap-2 mb-2 opacity-80">
                                <DiamondIcon className="w-3.5 h-3.5" />
                                <span className="text-[10px] font-bold uppercase tracking-widest">Viability</span>
                            </div>
                            <div>
                                <span className="text-base md:text-lg font-black uppercase tracking-wide block">{voyagePlan.suitability?.status}</span>
                                <span className="text-[10px] opacity-80 leading-tight mt-0.5">{voyagePlan.suitability?.reasoning || "Route analyzed."}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="w-full bg-slate-900 border border-white/10 rounded-3xl p-6 md:p-8 relative overflow-hidden shadow-xl">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <div>
                            <h3 className="text-lg font-bold text-white uppercase tracking-widest flex items-center gap-2">
                                <ServerIcon className="w-5 h-5 text-sky-400" /> Comprehensive Voyage Log
                            </h3>
                            <p className="text-xs text-slate-400 font-medium">Detailed Telemetry & Environmental Esimates</p>
                        </div>
                        {/* Summary Timing */}
                        {voyagePlan.bestDepartureWindow && (
                            <div className="bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-lg flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">Best Departure</div>
                                    <div className="text-white font-mono text-xs">{voyagePlan.bestDepartureWindow.timeRange}</div>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="text-[10px] text-gray-500 uppercase tracking-widest border-b border-white/10">
                                    <th className="pb-4 pl-2 font-bold">Checkpoint / ETA</th>
                                    <th className="pb-4 font-bold">Position</th>
                                    <th className="pb-4 font-bold">Wind</th>
                                    <th className="pb-4 font-bold">Sea State</th>
                                    <th className="pb-4 font-bold">Notes</th>
                                </tr>
                            </thead>
                            <tbody className="text-xs md:text-sm font-mono text-gray-300">
                                ...
                                <tr className="border-b border-white/5 group hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="py-4 text-gray-500 italic">--</td>
                                    <td className="py-4 text-gray-500 italic">--</td>
                                ...
                                {voyagePlan.waypoints.map((wp, i) => {
                                    // Calculate approx time based on accumulated distance if possible
                                    // For simplicity, we assume linear progression. 
                                    // A robust impl would accumulate actual leg distances.
                                    // Let's calculating Leg Distance from previous point for ETA.
                                    ...
                                    const hours = (distNm / (vessel.cruisingSpeed || 5)); // Leg Hours
                                ...
                                <tr className="group hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="py-4 text-gray-500 italic">--</td>
                                    <td className="py-4 text-gray-500 italic">--</td>
                                    <td className="py-4 text-emerald-400 font-bold text-xs uppercase tracking-wider">
                                        Destination Reach
                                    </td>
                                </tr>
                            </tbody>
                ...
                {/* 2. CHART ACTION BAR (Full Width) */}
                <button
                    onClick={() => setIsMapOpen(true)}
                    aria-label="Open Interactive Chart"
                    className="w-full h-[120px] bg-slate-800 border border-white/10 rounded-3xl overflow-hidden relative group cursor-pointer shadow-xl transition-all hover:border-sky-500/30 flex items-center justify-between px-8 md:px-12"
                >
                    ...
                    <div className="relative z-10 flex flex-col items-start gap-1">
                        <h3 className="text-xl md:text-2xl font-bold text-white shadow-black drop-shadow-md">Interactive Chart</h3>
                    ...
                    <div className="relative z-10 bg-sky-500/90 p-4 rounded-full text-white shadow-lg group-hover:scale-110 transition-transform group-hover:bg-sky-500 border border-white/20">
            ...
            {/* INTELLIGENCE STACK - VERTICAL */}
            <div className="flex flex-col gap-6 mb-8">

                {/* 1. Optimal Departure Window */}
                <div className="w-full">
                    {voyagePlan.bestDepartureWindow ? (
                        <div className="bg-emerald-900/20 border border-emerald-500/20 rounded-3xl p-6 md:p-8 relative overflow-hidden flex flex-col md:flex-row gap-6 items-center shadow-lg">
                            <div className="absolute top-0 right-0 p-24 bg-emerald-500/5 rounded-full blur-3xl pointer-events-none -translate-y-1/2 translate-x-1/2"></div>
                        ...
                        <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-6 flex items-center justify-center">
                            <span className="text-xs text-gray-500 font-bold uppercase tracking-widest">No Departure Timing Data Available</span>
                        </div>
                    )}
                </div>

                {/* 2. AI Deep Analysis Block - EXPANDED */}
                <div className="bg-gradient-to-br from-indigo-900/40 to-slate-900/95 border border-indigo-500/20 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden flex flex-col">
                    <div className="flex justify-between items-start mb-6 border-b border-indigo-500/20 pb-4">
                        <div>
                            <h3 className="text-lg font-bold text-indigo-200 uppercase tracking-widest flex items-center gap-2 mb-1">
                                <DiamondIcon className="w-5 h-5 text-indigo-400" /> Deep Voyage Analysis
                            </h3>
                            <p className="text-xs text-indigo-400/60 font-medium">AI-Generated Strategy & Safety Assessment</p>
                        </div>
                        ...
                        {!deepReport && (
                            <button
                                ...
                                className="text-[10px] font-bold uppercase tracking-wider text-indigo-400 hover:text-white border border-indigo-500/30 px-4 py-2 rounded-lg hover:bg-indigo-500/20 transition-all flex items-center gap-2"
                            >
                                {analyzingDeep ? <div className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" /> : "Analyze Route"}
                            </button>
                        )}
                    </div>
                    ...
                    <div className="flex-1">
                        {deepReport ? (
                            ...
                            <div className="bg-black/20 rounded-xl p-8 border border-white/5 text-center flex flex-col items-center justify-center min-h-[200px]">
                                ...
                                <button onClick={handleDeepAnalysis} className="text-indigo-400 hover:text-indigo-300 underline text-xs font-bold uppercase tracking-wider">Run Deep Analysis for Comprehensive Report</button>
                            </div>
                        )}
                    </div>
                </div>

                {/* FUEL & PROVISIONS CALCULATOR */}
                <ResourceCalculator voyagePlan={voyagePlan} vessel={vessel} crewCount={2} />
            ...
            {/* MAIN CONTENT STACK - VERTICAL FLOW */}
            <div className="flex flex-col gap-8 flex-1">

                {/* SECTION 1: SUGGESTED STOPS (Wide Timeline) */}
                <div className="bg-slate-900/80 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-sky-500/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none transition-opacity group-hover:opacity-100 opacity-50"></div>

                    <div className="flex justify-between items-end mb-6 pb-4 border-b border-white/5 relative z-10">
                        <div>
                            <h3 className="text-lg font-bold text-white uppercase tracking-widest flex items-center gap-2 mb-1">
                                <RouteIcon className="w-5 h-5 text-sky-400" /> Suggested Route Plan
                            </h3>
                            <p className="text-xs text-slate-400 font-medium">Tactical Routing & Stops</p>
                        </div>
                        <span className="text-xs font-mono text-sky-500 bg-sky-500/10 px-2 py-1 rounded border border-sky-500/20">{(voyagePlan.waypoints?.length || 0)} NODES</span>
                    </div>
                    ...
                    <div className="relative z-10">
                        {(voyagePlan.waypoints && voyagePlan.waypoints.length > 0) ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                {voyagePlan.waypoints.map((wp, i) => (
                                    <div key={i} className="flex gap-4 relative group/node cursor-pointer select-none bg-white/5 hover:bg-white/10 border border-white/5 hover:border-sky-500/30 rounded-xl p-4 transition-all" onClick={() => setIsMapOpen(true)}>
                                        {/* Node Number Badge */}
                                        <div className="absolute -top-2 -right-2 w-6 h-6 bg-slate-800 border border-white/10 rounded-full flex items-center justify-center text-[9px] font-mono text-gray-500 shadow-lg group-hover/node:border-sky-500/50 group-hover/node:text-sky-400 transition-colors">
                                            {i + 1}
                                        </div>

                                        {/* Serial Icon */}
                                        <div className="mt-1">
                                            <div className="w-8 h-8 rounded-full bg-slate-900 border-2 border-sky-500/30 flex items-center justify-center shrink-0 shadow-lg shadow-sky-900/10 group-hover/node:scale-110 transition-transform">
                            ...
                            <div className="flex flex-col items-center justify-center py-12 opacity-50 border-2 border-dashed border-white/10 rounded-2xl bg-white/5">
                ...
                {/* SECTION 2: HAZARDS (Wide Matrix) */}
                <div className="bg-slate-900/80 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-red-500/5 rounded-full blur-3xl -translate-y-1/2 -translate-x-1/2 pointer-events-none"></div>
                    ...
                    <div className="relative z-10">
                        <h3 className="text-lg font-bold text-white uppercase tracking-widest flex items-center gap-2 mb-6 border-b border-white/5 pb-4">
                            <BugIcon className="w-5 h-5 text-red-500" /> Hazard Identification
                        </h3>

                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            {voyagePlan.hazards && voyagePlan.hazards.length > 0 ? (
                                voyagePlan.hazards.map((h, i) => (
                                    <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-red-500/5 border border-red-500/10 hover:bg-red-500/10 hover:border-red-500/30 transition-all group h-full">
                                ...
                                <div className="col-span-full py-12 flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-2xl bg-white/5 opacity-60">
                ...
                {/* SECTION 3: SYSTEMS CHECK (Wide Grid) */}
                <div className="bg-slate-900/80 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
                    <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-orange-500/5 rounded-full blur-3xl translate-y-1/2 translate-x-1/2 pointer-events-none"></div>

                    <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-6 pb-4 border-b border-white/5 relative z-10">
                        <div>
                            <h3 className="text-lg font-bold text-white uppercase tracking-widest flex items-center gap-2 mb-1">
                                <GearIcon className="w-5 h-5 text-orange-400" /> Pre-Departure Systems
                            </h3>
                            <p className="text-xs text-slate-400 font-medium">Internal Vessel Integrity</p>
                        </div>

                        {/* Centered Tabbed Checklist for Wide Mode */}
                        <div className="flex bg-black/40 p-1.5 rounded-xl border border-white/5">
                            {CHECKLIST_DATA.map((cat) => (
                                <button
                                    key={cat.id}
                                    onClick={() => setActiveChecklistTab(cat.id)}
                                    className={`px-4 py-2 text-[10px] md:text-xs font-bold uppercase rounded-lg transition-all ${activeChecklistTab === cat.id ? 'bg-white/10 text-white shadow-lg border border-white/10' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}
                                >
                                    {cat.category}
                                </button>
                            ))}
