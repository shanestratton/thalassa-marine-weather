...
export function useChatMessages(options: UseChatMessagesOptions) {
    ...
    const sendChannelMessage = useCallback(async (bypassFilter = false) => {
    ...
    const handleMarkHelpful = useCallback(async (msgId: string) => {
        ...
        setMessages(prev => prev.map(m =>
            m.id === msgId ? { ...m, helpful_count: m.helpful_count + 1 } : m
        ));
    ...
    return {
        ...
        showModMenu, setShowModMenu,
        showRankTooltip, setShowRankTooltip,
