async function test() {
    ...
    console.log("Fetching URL:", url.replace(mapboxKey, 'HIDDEN'));
    ...
    if (data.features && data.features.length > 0) {
        for (const place of data.features) {
            ...
            const isGenericContext = name.startsWith("Location") ||
                /^[+-]?\d/.test(name) ||
                /\b(Ocean|Sea|Reef)\b/i.test(name);
            console.log("isGenericContext (openmeteo rule):", isGenericContext);
        }
    } else {
